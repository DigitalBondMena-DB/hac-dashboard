...
@Component({
  ...
  templateUrl: './tamara-orders.component.html',
  styleUrls: ['./tamara-orders.component.scss'],
  providers: [MessageService],
})
export class TamaraOrdersComponent implements OnInit {
  orders: Order[] = [];
  ...
  loadOrders(): void {
    this.ordersService.getAllOrders().subscribe({
      next: (data: OrdersListResponse) => {
        ...
        // Filter out cancelled, delivered orders, and non-Tamara payment methods
        let filteredData = data.orders.filter(
          (order) => 
            order.order_status !== 'cancelled' && 
            order.order_status !== 'delivered' && 
            order.payment_method === 'Tamara'
        );
